...
This means that it simply calls `.toString()` on a JSX node.
...
Ok so first instinct was to run a `ps` and see what was running. Not a lot
...
Some logs come through. They tried.
...
Grann described the logs that the ships would take. Remining me to keep logging here.

Thinking about how those kimds of logs too have been automated in technology.
...
But the logs that he drew from were created by people. They included the sentiment of the crew and synthesized the raw data with the human experience at the tine of recording.
